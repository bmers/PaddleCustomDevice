...
std::vector<paddle::Tensor> FFNOp(
...
// #ifdef PADDLE_WITH_ASCEND_TRANSFORMER_ACC
  ...
  auto out_shape = FFNOpInferShape(
  ...
  NPUAttributeMap attrs = {{"transpose_x1", transpose_x1}, {"transpose_x2", transpose_x2}};
  ...
  const auto& runner2 =
      NpuOpRunner("Add", {*matmul_res, *b}, {*add_res});
  runner2.Run(stream);
  
  const auto& runner3 =
      NpuOpRunner("Gelu", {*add_res}, {*out}, {});
  runner3.Run(stream);
  
  
  // post
  auto out_dims = FFNOpInferShape(
      input.shape(), weight.shape(), bias.shape()).at(0);
