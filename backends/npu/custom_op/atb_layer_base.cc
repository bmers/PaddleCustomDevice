...
void PpAscendAtbLayerOpBase::BuildVariantPack(std::vector<const phi::DenseTensor *> &inTensors,
...
void PpAscendAtbLayerOpBase::SetWorkspace(uint64_t workspace_size)
{
...
atb::Status PpAscendAtbLayerOpBase::Execute(aclrtStream stream,
                                     ...
                                     std::vector<const phi::DenseTensor *> &outTensors)
{
  ...
  st = operation_->Execute(variantPacks_, workspace_, workspace_size, stream);

  return st;
}

PpAscendAtbLayerOpBase::PpAscendAtbLayerOpBase(const std::string &opName,
                                               int32_t layerNum)
{
  opName_ = opName;
  layerNum_ = layerNum;
...
PpAscendAtbLayerOpBase::~PpAscendAtbLayerOpBase() {}
