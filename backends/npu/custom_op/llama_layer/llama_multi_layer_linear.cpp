...
atb::Status CreateLlamaMultiLayerLinearOperation(const MultiLayerLinearParam &param, atb::Operation **operation)
{
    ...
    linearNode.inTensorReshapeFuncs.resize(linearNode.size());
    linearNode.inTensorReshapeFuncs[0] = [](const atb::Dim &oldShape, atb::Dims &newShape) {
      newShape.dim = 2; // dimNum: 2
